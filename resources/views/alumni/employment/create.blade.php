...
<div class="bg-gray-50 min-h-screen py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            ...
            <h1 class="text-3xl font-bold text-gray-900">Tambah Data Pekerjaan</h1>
            <p class="mt-2 text-sm text-gray-600">Lengkapi informasi pekerjaan atau kegiatan Anda saat ini</p>
        </div>

        <!-- Form -->
        <form action="{{ route('alumni.employment.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <label class="relative flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all hover:border-blue-300 has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                        <input type="radio" name="employment_status" value="employed" class="peer sr-only" {{ old('employment_status') == 'employed' ? 'checked' : '' }} required>
                    ...
                    <label class="relative flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all hover:border-purple-300 has-[:checked]:border-purple-500 has-[:checked]:bg-purple-50">
                        <input type="radio" name="employment_status" value="entrepreneur" class="peer sr-only" {{ old('employment_status') == 'entrepreneur' ? 'checked' : '' }}>
                        <div class="flex items-center">
                    ...
                    <label class="relative flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all hover:border-yellow-300 has-[:checked]:border-yellow-500 has-[:checked]:bg-yellow-50">
                        <input type="radio" name="employment_status" value="studying" class="peer sr-only" {{ old('employment_status') == 'studying' ? 'checked' : '' }}>
                        <div class="flex items-center">
                    ...
                    <label class="relative flex items-center p-4 rounded-lg border-2 cursor-pointer transition-all hover:border-gray-300 has-[:checked]:border-gray-500 has-[:checked]:bg-gray-50">
                        <input type="radio" name="employment_status" value="unemployed" class="peer sr-only" {{ old('employment_status') == 'unemployed' ? 'checked' : '' }}>
                        <div class="flex items-center">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <div class="relative">
                            <input type="text" name="company_name" id="company_name" value="{{ old('company_name') }}"
                                   ...
                                   autocomplete="off">
                            <input type="hidden" id="selected_employer_id" name="employer_id" value="">
                    ...
                    <div>
                        ...
                        <select name="industry_type" id="industry_type"
                            ...
                            <option value="Technology" {{ old('industry_type') == 'Technology' ? 'selected' : '' }}>Teknologi/IT</option>
                            <option value="Finance" {{ old('industry_type') == 'Finance' ? 'selected' : '' }}>Keuangan/Perbankan</option>
                            <option value="Education" {{ old('industry_type') == 'Education' ? 'selected' : '' }}>Pendidikan</option>
                            <option value="Healthcare" {{ old('industry_type') == 'Healthcare' ? 'selected' : '' }}>Kesehatan</option>
                            <option value="Manufacturing" {{ old('industry_type') == 'Manufacturing' ? 'selected' : '' }}>Manufaktur</option>
                            <option value="Retail" {{ old('industry_type') == 'Retail' ? 'selected' : '' }}>Retail/Perdagangan</option>
                            <option value="Construction" {{ old('industry_type') == 'Construction' ? 'selected' : '' }}>Konstruksi</option>
                            <option value="Transportation" {{ old('industry_type') == 'Transportation' ? 'selected' : '' }}>Transportasi/Logistik</option>
                            <option value="Hospitality" {{ old('industry_type') == 'Hospitality' ? 'selected' : '' }}>Hotel/Pariwisata</option>
                            <option value="Media" {{ old('industry_type') == 'Media' ? 'selected' : '' }}>Media/Komunikasi</option>
                            <option value="Government" {{ old('industry_type') == 'Government' ? 'selected' : '' }}>Pemerintahan</option>
                            <option value="Other" {{ old('industry_type') == 'Other' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        @error('industry_type')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="job_level" id="job_level"
                            ...
                            <option value="lead" {{ old('job_level') == 'lead' ? 'selected' : '' }}>Lead</option>
                            ...
                            <option value="vp" {{ old('job_level') == 'vp' ? 'selected' : '' }}>Vice President</option>
                            <option value="ceo" {{ old('job_level') == 'ceo' ? 'selected' : '' }}>C-Level (CEO/CTO/CFO/dll)</option>
                        </select>
                        @error('job_level')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="contract_type" id="contract_type"
                            ...
                            <option value="contract" {{ old('contract_type') == 'contract' ? 'selected' : '' }}>Contract/Project Based</option>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="job_description" id="job_description" rows="4"
                                  ...
                                  placeholder="Jelaskan tanggung jawab dan tugas utama Anda...">{{ old('job_description') }}</textarea>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="study_level" id="study_level"
                            ...
                            <option value="S2" {{ old('study_level') == 'S2' ? 'selected' : '' }}>S2 (Master)</option>
                            <option value="S3" {{ old('study_level') == 'S3' ? 'selected' : '' }}>S3 (Doktor)</option>
                            <option value="Profesi" {{ old('study_level') == 'Profesi' ? 'selected' : '' }}>Profesi</option>
                            <option value="Spesialis" {{ old('study_level') == 'Spesialis' ? 'selected' : '' }}>Spesialis</option>
                            <option value="Lainnya" {{ old('study_level') == 'Lainnya' ? 'selected' : '' }}>Lainnya</option>
                        </select>
                        @error('study_level')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    companyNameInput.addEventListener('change', function() {
        // If user changes the value after selecting, clear the employer_id
        const currentValue = this.value;
        const selectedValue = selectedEmployerId.value;
        
        if (selectedValue && currentValue) {
            // Check if the current value matches the selected employer
            // If not, clear the employer_id to allow creating new employer
            setTimeout(() => {
