                        ...
                        <div class="ml-4">
                            ...
                            @elseif($canStart && !$session->is_expired)
                                @if($status === 'partial' || $status === 'draft')
                                    <a href="{{ route('alumni.survey.questionnaire', $response) }}" class="inline-flex items-center px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm">
                                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
